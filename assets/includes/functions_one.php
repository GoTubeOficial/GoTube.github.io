...
function PT_ImportImageFromLogin($media) {
    ...
    if (file_exists($file_dir)) {
        if ($pt->config->s3_upload == 'on') {
            $file_dir = PT_UploadToS3($file_dir);
        }
        return $file_dir;
    } else {
...
function pt_push_channel_notifiations($video_id = 0) {
    ...
    foreach ($get_subscribers as $key => $subscriber) {
        $userIds[] = "('{$pt->user->id}', '{$subscriber->subscriber_id}', 'added_video', 'watch/{$video_id}', '" . time() . "')";
    }
    $query_implode       = implode(',', $userIds);
    $query_row           = $db->rawQuery("INSERT INTO " . T_NOTIFICATIONS . " (`notifier_id`, `recipient_id`, `type`, `url`, `time`) VALUES $query_implode");
    if ($query_row) {
        return true;
    }
}
...
function PT_GetMessages($id, $data = array()) {
    global $pt, $db;
    if (IS_LOGGED == false) {
        return false;
    }
    ...
    $count_user_messages = $count_user_messages - 50;
    ...
    $get_user_messages = $db->orderBy('id', 'ASC')->get(T_MESSAGES, array($count_user_messages, 50));
